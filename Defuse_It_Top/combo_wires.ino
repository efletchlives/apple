...
RangedInt WIRE_BLUE(5000, 9000, 7000);   // on breadboard: BLUE#include <Wire.h>
#include "ADS1X15.h"
#include "Voltage_Value.h"
#include <Adafruit_SSD1306.h>
...
int combos[6][3] = {{0,1,2},  // [i][1] = Wire # to green port,                   
                    {0,2,1},  // [i][2] = Wire # to blue port, 
                    {1,0,2},  // [i][3] = Wire # to black port
                    {1,2,0},
                    {2,0,1},
                    {2,1,0}};
...
bool wiresLoop(double timer, bool &wire_in) {
  // sound choice
  if(wire_in == 0) {
    speaker.play(6); // connect wires clip
  }
  else {
    speaker.play(7); // remove wires clip
  }
